...
// 8354-brothers-and-sisters
    async createMedia(data, genres = []) {
        ...
        try {
            ...
            if (Array.isArray(genres) && genres.length > 0) {
                ...
                const genrePlaceholders = genres.map((_, i) => `($1, $${i + 2})`).join(", ");
    ...
    async updateMediaById(id, { media = {}, genres } = {}) {
        ...
        try {
            ...
            if (Object.keys(media).length > 0) {
                const idxMedia = Object.keys(media).map((f, i) => `${f} = $${i + 1}`).join(", ");
            ...
            if ( genres !== undefined) {
                ...
                if (genres.length > 0){
                    ...
                    const placeholders = genres.map((_, i) => `($1, $${i + 2})`).join(", ");
                    const res = await client.query(
  ...
  // findAllMedia
    async findAllMedia({ category, page = 1, limit = 20, search, sortBy = 'created_at', sortOrder = 'DESC' }) {
        const queryParams = [];
